...
const ShowMore:React.FC<props> = (show_sidebar) => {
  ...
  const employee:employee[] = JSON.parse(localStorage.getItem("employees") || "[]")
  ...
  departments.forEach((element) => {
    var cnt:employee[]  = employee.filter((emp) => emp.Dept_Name == element);
    departments_count.push(cnt.length);
  });
  offices.forEach((element) => {
    var cnt:employee[]  = employee.filter((emp) => emp.Office_Details == element);
    offices_count.push(cnt.length);
  });
  job_titles.forEach((element) => {
    var cnt:employee[]  = employee.filter((emp) => emp.Title == element);
